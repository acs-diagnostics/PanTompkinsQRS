...
/**
 * @brief Use this function for any kind of setup you need before getting
 * samples. This is a good place to open a file, initialize your hardware and/or
 * open a serial connection. Remember to update its parameters on the
 * panTompkins.h file as well.
 *
 * @param file_in
 * @param file_out
 */
...
/**
 * @brief Use this function to output the information you see fit (last
 * RR-interval, sample index which triggered a peak detection, whether each
 * sample was a R peak (1) or not (0) etc), in whatever way you see fit (write
 * on screen, write on file, blink a LED, call other functions to do other kinds
 * of processing, such as feature extraction etc). Change its parameters to
 * receive the necessary information to output.
 *
 * @param out
 */
void output(int out)
...
/**
 * @brief This is the actual QRS-detecting function. It's a loop that
 * constantly calls the input and output functions and updates the thresholds
...
void panTompkins()
{
  ...
  dataType signal[BUFFSIZE], derivative[BUFFSIZE], squared[BUFFSIZE], integral[BUFFSIZE], outputSignal[BUFFSIZE];
  ...
  int rr1[8] = {0}, rr2[8] = {0}, rravg1 = 0, rravg2 = 0, rrlow = 0, rrhigh = 0, rrmiss = 0;
  ...
  short input_buffer[1] = {0};

  int read_result = 0;

  read_result = fread(input_buffer, sizeof(short), 1, fin);
  ...
  while (read_result == 1) {
    ...
    derivative[current] = signal[current];
    if (current > 0) {
      derivative[current] -= signal[current - 1];
    }
    ...
    if (integral[current] >= threshold_i1 || signal[current] >= threshold_f1) {
      peak_i = integral[current];
      peak_f = signal[current];
    }
